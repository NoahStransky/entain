...
2. In a terminal window, start our racing and sports service...

```bash
cd ./racing

go build && ./racing
➜ INFO[0000] racing gRPC server listening on: localhost:9000
```
...
go build && ./sports
➜ INFO[0000] sports gRPC server listening on: localhost:9001
```
...
```bash
curl -X "GET" "https://localhost:8000/v1/races/1" \
       -H 'Content-Type: application/json'
```
...
curl -X "POST" "http://localhost:8000/v1/list-events" \
     ...
     -d $'{
  "filter": {
        "sortBy": "advertised_start_time desc"
  }
}'

```

5. open different terminal window and run test
